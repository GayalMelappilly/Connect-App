...
const MdUserSearch = ({ setMdShowUserSearch, setMdProp }) => {
    ...
    useEffect(() => {
        axios.get(`${import.meta.env.VTIE_APP_NODE_ENV === 'development' ? import.meta.env.VTIE_APP_LOCAL_URI : import.meta.env.VTIE_APP_RENDER_URI}/user/list?search=${username}&id=${userInfo._id}`).then((response) => {
    ...
    const handleAddFriend = (user) => {
        axios.post(`${import.meta.env.VTIE_APP_NODE_ENV === 'development' ? import.meta.env.VTIE_APP_LOCAL_URI : import.meta.env.VTIE_APP_RENDER_URI}/user/add-friend`, { senderDetails: userInfo, receiverDetails: user }).then((response) => {
            console.log("ADDED : ", response.data)
        })
    }

    const handleAccept = (user) => {
        axios.put(`${import.meta.env.VTIE_APP_NODE_ENV === 'development' ? import.meta.env.VTIE_APP_LOCAL_URI : import.meta.env.VTIE_APP_RENDER_URI}/user/req-accept`, { reqFrom: user, reqTo: userInfo }).then((response) => {
    ...
    const handleDecline = (user) => {
        axios.put(`${import.meta.env.VTIE_APP_NODE_ENV === 'development' ? import.meta.env.VTIE_APP_LOCAL_URI : import.meta.env.VTIE_APP_RENDER_URI}/user/req-decline`, { reqFrom: user, reqTo: userInfo }).then((response) => {
    ...
    const handleInvite = (username) => {
        ...
        if (username.includes('@gmail.com')) {
            setEmailCheck(true)
            axios.post(`${import.meta.env.LOCAL_URI}/user/invite-user`, { email: username, user: userInfo }).then((response) => {
